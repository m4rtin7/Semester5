...
const (
	windowWidthMedium, windowHeightMedium = 1024, 800
	ballSpeed                 = 8
	panelSpeed                = 7
	ballRadius                = 5
)
...
func main() {
	...
	resetAfterScore := func(ballVelocitySign float32) {
		ball = circle{
			centerX: windowWidthMedium / 2,
			centerY: windowHeightMedium / 2,
			radius:  ballRadius,
			vx:      ballVelocitySign * ballSpeed,
			vy:      0.0,
		}
		leftPanel = rect{10, windowHeightMedium/2 - 40, 10, 80}
		rightPanel = rect{windowWidthMedium - 20, windowHeightMedium/2 - 40, 10, 80}
	}
	resetAfterScore(1.0)

	const title = "Tennis - press N to restart"
	err := draw.RunWindow(title, windowWidthMedium, windowHeightMedium, func(window draw.Window) {
		...
		window.FillRect(0, 0, windowWidthMedium, windowHeightMedium, draw.DarkGreen)
		window.DrawRect(50, 50, windowWidthMedium-100, windowHeightMedium-100, draw.LightGreen)
		window.DrawLine(windowWidthMedium/2, 50, windowWidthMedium/2, windowHeightMedium-50, draw.LightGreen)
...
func (r *rect) clampInY() {
	...
	if r.y+r.h >= windowHeightMedium {
		r.y = windowHeightMedium - r.h
	}
}
...
func (c *circle) collideWall() bool {
	if c.centerY < float32(c.radius) ||
		c.centerY+float32(c.radius) >= windowHeightMedium {
		c.vy = -c.vy
		return true
	}
	return false
...
func (c *circle) isInRightGoal() bool {
	return c.centerX > float32(windowWidthMedium+c.radius)
}
...
func drawScore(left, right int, window draw.Window) {
	...
	window.DrawScaledText(scoreText, (windowWidthMedium-w)/2+1, 10, scale, draw.Black)
}
